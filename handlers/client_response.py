...
# Состояние для сбора комментария от клиента
...
async def client_accept(c):
    ...
    # переводим заявку в «new» для выставления счёта
...
async def client_reject(c):
    ...
    # убираем кнопки подтверждения
    await bot.edit_message_reply_markup(c.message.chat.id, c.message.message_id, reply_markup=None)
    await bot.send_message(c.message.chat.id, "❗ Напишите, пожалуйста, комментарий к ответу:")
    await bot.answer_callback_query(c.id)
...
async def client_comment(m):
    ...
    # сохраняем комментарий в данных заявки
    o = get_order(order_id)
    d = o.data
    d.setdefault("comments", []).append(comment)
    update_order_data(order_id, d)
    # переводим заявку обратно в «new»
    update_order_status(order_id, "new")

    await bot.send_message(m.chat.id, "❗ Ваш комментарий отправлен администратору.")
    await bot.send_message(
        config.ADMIN_CHAT_ID,
